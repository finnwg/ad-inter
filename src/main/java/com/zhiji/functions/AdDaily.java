...
public class AdDaily {
    ...
    public static void adDaily(Properties properties) throws IOException, SQLException, ClassNotFoundException {

        //get请求url
        httpUrl = properties.getProperty("http.qq.daily.url");
        //开始结束时间
        ...
        //get请求url参数
        groupBy = properties.getProperty("http.qq.daily.group_by");
        level = properties.getProperty("http.qq.daily.level");
        timeLine = properties.getProperty("http.qq.daily.time_line");
        pageSize = properties.getProperty("http.qq.daily.page_size");
        fields = properties.getProperty("http.qq.daily.fields");


        //目标表名称
        goalTable = properties.getProperty("http.qq.daily.table");



        //过去token和accountId，遍历
        tokenAndAccountId = properties.getProperty("http.qq.daily.token&account_id").split(",");
        for (int i = 0; i < tokenAndAccountId.length; i++) {
            ...
            if (!"{}".equals(jsonNodes.get("data").toString())) {
                ...
                for (int j = 1; j < total_page + 1 ; j++) {
                    ...
                    //重新生成nonce
                    nonce = (nonce + timestamp).hashCode() + "";
